...
let NatureQuestions: [Question] = [
    Question(
        text: "What is the dominant climate in your world?",
        answers: [
            Answer(text: "Desert",scoreValue: 5),
            Answer(text: "Waters and Oceans",scoreValue: 0),
            Answer(text: "Jungles & Rainforests",scoreValue: 10),
            Answer(text: "Mixed",scoreValue: 15)
        ]
    ),
    
    Question(
        text: "In your world, what form of life is most common?",
        answers: [
            Answer(text: "Animals",scoreValue: 10),
            Answer(text: "Aquatic life",scoreValue: 0),
            Answer(text: "Technological life",scoreValue: 15),
            Answer(text: "Plants",scoreValue: 5)
        ]
    ),
    
    Question(
        text: "How aggressive is nature in your planet?",
        answers: [
            Answer(text: "Calm",scoreValue: 10),
            Answer(text: "Predictable and Mild",scoreValue: 15),
            Answer(text: "Unpredictable",scoreValue: 5),
            Answer(text: "Very Hostile",scoreValue: 0)
        ]
    ),
    
    Question(
        text: "Has nature adapted to humans?",
        answers: [
            Answer(text: "No" ,scoreValue: 0),
            Answer(text: "Yes",scoreValue: 10),
...
struct NatureCategoryView: View {
    ...
    var onFinish: () -> Void
    let questions: [Question] = NatureQuestions
    ...
    func resetCategory() {
        withAnimation {
            ...
            worldManager.natureScore = 0 
        }
    }
    func goToNextStep() {
        if let selectedAnswer = worldChoices[currentQuestion.id]{
            worldManager.natureScore += selectedAnswer.scoreValue
        }
        if currentQuestionIndex < questions.count - 1 {
            ...
            onFinish()
             print(worldManager.natureScore)
               worldManager.completedCategories.append("4")
            dismiss() 
        }
    }    
